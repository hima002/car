package com.hima.alwarsha.service
...
import com.hima.alwarsha.receiver.ActivityTransitionReceiver
 ...
 * [ActivityTransitionReceiver], which relays start/stop actions here. Location updates only run
 * while IN_VEHICLE is active, saving battery the rest of the time. Consecutive GPS fixes are
 * filtered for accuracy and implausible speed jumps, then accumulated distance is flushed to
 * [CarRepository] in small batches (not on every fix) to limit database writes.
 */
class DrivingTrackingService : Service() {
    ...
    private fun registerActivityTransitions() {
        ...
        activityRecognitionClient.requestActivityTransitionUpdates(
            ActivityTransitionRequest(transitions), transitionPendingIntent()
        )
    }
    ...
    private fun transitionPendingIntent(): PendingIntent {
        ...
        return PendingIntent.getBroadcast(
            this, REQUEST_CODE_TRANSITION, intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_MUTABLE
        )
    }
    ...
    private fun startLocationUpdates() {
        ...
        val request = LocationRequest.Builder(
            Priority.PRIORITY_BALANCED_POWER_ACCURACY, TimeUnit.SECONDS.toMillis(LOCATION_INTERVAL_SECONDS)
        )
    ...
    private fun processLocation(location: Location) {
        ...
        lastAcceptedLocation = location
        if (speedKmh > MAX_PLAUSIBLE_SPEED_KMH) return // implausible GPS jump — reset baseline, don't count it

        accumulatedKm += distanceMeters / 1000.0
        if (accumulatedKm >= MIN_KM_BEFORE_PERSIST) flushAccumulatedDistance()
    }
    ...
    companion object {
        const val ACTION_START_TRACKING = "com.hima.alwarsha.action.START_TRACKING"
        const val ACTION_STOP_TRACKING = "com.hima.alwarsha.action.STOP_TRACKING"
        const val ACTION_START_LOCATION_UPDATES = "com.hima.alwarsha.action.START_LOCATION_UPDATES"
        const val ACTION_STOP_LOCATION_UPDATES = "com.hima.alwarsha.action.STOP_LOCATION_UPDATES"
